...
class OverpassPlaceMapper {
  const OverpassPlaceMapper();
  ...
  Place fromJson(Map<String, dynamic> json) {
    ...
    final typeLabel = _typeLabel(inferredType);

    final addrFull = tags['addr:full']?.toString();
    final street =
        (tags['addr:street'] ?? tags['contact:street'] ?? tags['addr:place'])
            ?.toString();
    final house = tags['addr:housenumber']?.toString();
    final city = (tags['addr:city'] ?? tags['addr:place'])?.toString();

    final nameRu = tags['name:ru']?.toString();
    ...
    return Place(
      ...
      type: inferredType,
      rating: _parseRating(tags['rating']),
  ...
  static List<String> _inferTypes({
    ...
    if (inferred.isEmpty) return const [];
